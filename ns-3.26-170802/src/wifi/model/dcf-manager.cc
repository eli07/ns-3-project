...
DcfManager::IsWithinAifs (DcfState *state) const
{
  ...
  Time ifsEnd = GetAccessGrantStart () + MicroSeconds (state->GetAifsn () * m_slotTimeUs);
	...
	if(m_low->GetIfIndex() == 1) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->eetAddress() << " <" << m_low->GetIfIndex() << "> ifsEnd: " << ifsEnd);
	}
	*/
...
// NETSYS ----------------------------------------------------------------------
void
DcfManager::RequestReaccess(DcfState *state)
{
  	NS_LOG_FUNCTION (this << state);
	...
	if(m_low->GetDebug()) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> RequestReaccess" );
	}
...
DcfManager::RequestAccess(DcfState *state)
{
	...
	if(m_low->GetDebug()) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> RequestAccess" );
	}
  ...
  if (state->GetBackoffSlots () == 0)
    {
		/*
		if(m_low->GetIfIndex() >= 0) {
			NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> RequestAccess BF:0 " << IsBusy() << " " << IsWithinAifs(state));
		}
		*/
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); k++)
    {
      DcfState *state = *i;
		...
		/*
		if(m_low->GetIfIndex() == 1 && state->GetAifsn() == 3) {
			NS_LOG_UNCOND(Simulator::Now() << " DoGrantAccess GetBackoffEndFor: " << GetBackoffEndFor(state));
		}
		*/
      ...
      if (state->IsAccessRequested () && GetBackoffEndFor (state) <= Simulator::Now () )
        {
          ...
          for (States::const_iterator j = i; j != m_states.end (); j++, k++)
            {
              ...
              if (otherState->IsAccessRequested () && GetBackoffEndFor (otherState) <= Simulator::Now ())
                {
           ...
           */
			if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> NotifyAccessGranted");
...
{
	//if(m_low->GetIfIndex() == 2) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> AccessTimeout");
  NS_LOG_FUNCTION (this);
  UpdateBackoff ();
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
...
DcfManager::GetBackoffStartFor (DcfState *state)
{
	...
	if(!(state->GetToken())) {
		mostRecentEvent = MostRecent (state->GetBackoffStart (),
                                     GetAccessGrantStart () + MicroSeconds (state->GetAifsn () * m_slotTimeUs));
	} else {
		mostRecentEvent = MostRecent (state->GetBackoffStart (),
                                     GetAccessGrantStart () + MicroSeconds (m_slotTimeUs));
	}
	//--------------------------------------------------------------------------
...
DcfManager::GetBackoffEndFor (DcfState *state)
{
  ...
  NS_LOG_DEBUG ("Backoff start: " << GetBackoffStartFor (state).As (Time::US) <<
    ...
    MicroSeconds (state->GetBackoffSlots () * m_slotTimeUs)).As (Time::US) << " slots: " << state->GetBackoffSlots()); 
...
DcfManager::UpdateBackoff (void)
{
  	...
  	for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++, k++)
    {
		...
		NS_LOG_DEBUG("Current backoff slots: " << state->GetBackoffSlots() << " backoffstart: " << backoffStart);
...
DcfManager::DoRestartAccessTimeoutIfNeeded (void)
{
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++)
	...
	//if(m_low->GetIfIndex() == 2) NS_LOG_UNCOND("expectedBackoffEnd: " << expectedBackoffEnd);
...
DcfManager::NotifyRxStartNow (Time duration)
{
  ...
  UpdateBackoff ();								// Backoff is updated here.
  ...
  m_rxing = true;
	...
	/*
	if(m_low->GetIfIndex() >= 0) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> NotifyRxStartNow");
  		for (States::iterator i = m_states.begin (); i != m_states.end (); i++) {
      		DcfState *state = *i;
      		uint32_t remainingSlots = state->GetBackoffSlots ();
			if(state->GetAifsn() == 3) NS_LOG_UNCOND("aifsn: " << state->GetAifsn() << " backoff slots: " << remainingSlots);
		}
	}
	*/
...
// NETSYS ----------------------------------------------------------------------
void
DcfManager::ResetBackoff(void) {
	for(States::iterator i = m_states.begin(); i != m_states.end(); i++) {
		DcfState *state = *i;
		state->ResetBackoffSlots();
	}
	DoRestartAccessTimeoutIfNeeded();
}
//------------------------------------------------------------------------------
...
DcfManager::NotifyRxEndOkNow (void)
{
	...
	/*
	if(m_low->GetIfIndex() == 2) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> NotifyRxEndOkNow");
		
  		for (States::iterator i = m_states.begin (); i != m_states.end (); i++)
   		{
   			DcfState *state = *i;
			if(state->GetAifsn() != 3) continue;
			if(state->GetToken()) {
				NS_LOG_UNCOND("I HAVE TOKEN BackoffEndFor: " << GetBackoffEndFor(state));
			}
		}
	}
	*/
	...
	/*
	// NETSYS ------------------------------------------------------------------
	// 1. Set the backoff counter to zero
	...
	//if(accessnow) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> ACCESS NOW");

	// 2. Reschedule AccessTimeout
  	DoRestartAccessTimeoutIfNeeded();
	//--------------------------------------------------------------------------
	*/
}
...
{
	//NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> NotifyRxEndErrorNow");
  	NS_LOG_FUNCTION (this);
