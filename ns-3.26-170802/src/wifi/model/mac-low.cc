...
// NETSYS ----------------------------------------------------------------------
void
MacLow::SetIfIndex(uint16_t ifIndex) {
	m_ifIndex = ifIndex;
}

void
MacLow::SetDebug(bool cmd) {
	m_debug = cmd;
}
...
// NETSYS ----------------------------------------------------------------------
void
MacLow::SetOverheardAckCallback(Callback<void> callback) {
  ...
  if (!m_currentHdr.IsQosData () && !m_currentHdr.IsBlockAck () && !m_currentHdr.IsBlockAckReq ())
      ...
      if (m_ampdu)
        {
          ...
          if (ampdu.GetRemainingNbOfMpdus () > 0)
			...
			//uint32_t mpdus = ampdu.GetRemainingNbOfMpdus();
			//if(GetIfIndex() == 0) NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " <" << GetIfIndex() << "> MacLow:StartTransmission mpdus: " << mpdus);
	...
	if(m_debug) {
		...
		if(m_ampdu) {
			AmpduTag ampdu;
			m_currentPacket->PeekPacketTag(ampdu);
			mpdus = ampdu.GetRemainingNbOfMpdus();
		}
		NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " <" << GetIfIndex() << "> MacLow:StartTransmission m_ampdu: " << m_ampdu << " mpdus: " << mpdus);
	}
...
{
	//NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " ReceiveOk");
  ...
  if (hdr.IsRts ())
	...
	// NETSYS ------------------------------------------------------------------
		...
		if(!m_overheardAckCallback.IsNull()) m_overheardAckCallback();
	}
	//--------------------------------------------------------------------------
          ...
          if (hdr.IsMgt () && ampduSubframe)
            {
              NS_FATAL_ERROR ("Received management packet as part of an A-MPDU");
				...
				//NS_LOG_UNCOND("UNICAST PACKET FOR ME");
            {
    ...
    {
		//NS_LOG_UNCOND("BROADCAST PACKET");
      if (ampduSubframe)
        {
