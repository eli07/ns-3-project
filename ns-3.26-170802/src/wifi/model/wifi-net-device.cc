...
WifiNetDevice::CompleteConfig (void)
{
  ...
  m_mac->SetLinkDownCallback (MakeCallback (&WifiNetDevice::LinkDown, this));
	// NETSYS ------------------------------------------------------------------
	m_mac->SetGotAckCallback(MakeCallback(&WifiNetDevice::GotAck, this));
	m_mac->SetMissedAckCallback(MakeCallback(&WifiNetDevice::MissedAck, this));
	m_mac->SetOverheardAckCallback(MakeCallback(&WifiNetDevice::OverheardAck, this));
	//--------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
void
WifiNetDevice::GotAck(void) {
	if(!m_gotAckCallback.IsNull()) {
		m_gotAckCallback(m_ifIndex);
	}
}

void
WifiNetDevice::MissedAck(void) {
	if(!m_missedAckCallback.IsNull()) {
		m_missedAckCallback(m_ifIndex);
	}
}

void
WifiNetDevice::OverheardAck(void) {
	if(!m_overheardAckCallback.IsNull()) {
		if(m_ifIndex == 0) m_overheardAckCallback(m_ifIndex);
	}
}
//------------------------------------------------------------------------------
...
void WifiNetDevice::SetGotAckCallback(WifiNetDeviceGotAckCallback callback) {
	m_gotAckCallback = callback;
}

void WifiNetDevice::SetMissedAckCallback(WifiNetDeviceMissedAckCallback callback) {
	m_missedAckCallback = callback;
}

void WifiNetDevice::SetOverheardAckCallback(WifiNetDeviceOverheardAckCallback callback) {
	m_overheardAckCallback = callback;
}

void WifiNetDevice::SetToken(bool cmd) {
	m_mac->SetToken(cmd);
}

void WifiNetDevice::SetDebug(bool cmd) {
	m_mac->SetDebug(cmd);
}

uint32_t WifiNetDevice::GetBEQueueSize() {
	return m_mac->GetBEQueueSize();
}
