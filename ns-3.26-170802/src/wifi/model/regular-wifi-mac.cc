  ...
  m_dsssSupported (0)
{
  ...
  m_low->SetRxCallback (MakeCallback (&MacRxMiddle::Receive, m_rxMiddle));
	...
	// NETSYS ------------------------------------------------------------------
	m_low->SetOverheardAckCallback(MakeCallback(&RegularWifiMac::OverheardAck, this));
	//--------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
	edca->SetGotAckCallback(MakeCallback(&RegularWifiMac::GotAck, this));
	edca->SetMissedAckCallback(MakeCallback(&RegularWifiMac::MissedAck, this));
//------------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
void
RegularWifiMac::SetGotAckCallback(GotAckCallback gotAckCallback) {
	NS_LOG_FUNCTION(this);
	m_gotAck = gotAckCallback;
}

void
RegularWifiMac::SetMissedAckCallback(MissedAckCallback missedAckCallback) {
	NS_LOG_FUNCTION(this);
	m_missedAck = missedAckCallback;
}

void
RegularWifiMac::SetOverheardAckCallback(OverheardAckCallback overheardAckCallback) {
	NS_LOG_FUNCTION(this);
	m_overheardAck = overheardAckCallback;
}
//------------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
void
RegularWifiMac::SetIfIndex(uint16_t ifIndex) {
	m_low->SetIfIndex(ifIndex);
}
//------------------------------------------------------------------------------
            ...
            case WifiActionHeader::BLOCK_ACK_ADDBA_REQUEST:
              {
				...
				if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> (RegularWifiMac) received ADDBA request from " << from);
...
{
	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> (RegularWifiMac) send ADDBA response to " << originator);
...
// NETSYS ----------------------------------------------------------------------
void
RegularWifiMac::GotAck() {
	if(!m_gotAck.IsNull()) m_gotAck();
}

void
RegularWifiMac::MissedAck() {
	if(!m_missedAck.IsNull()) m_missedAck();
}

void
RegularWifiMac::OverheardAck() {
	if(!m_overheardAck.IsNull()) m_overheardAck();
}

void
RegularWifiMac::SetToken(bool cmd) {

	GetBEQueue()->SetToken(cmd);
	/*
  	for (EdcaQueues::iterator i = m_edca.begin (); i != m_edca.end (); ++i) {
      i->second->SetToken(cmd);
    }
	*/
}

void
RegularWifiMac::SetDebug(bool cmd) {
	m_low->SetDebug(cmd);
}

uint32_t
RegularWifiMac::GetBEQueueSize() {
	return GetBEQueue()->GetQueueSize();
}
//------------------------------------------------------------------------------
