...
private:
  ...
  virtual void DoNotifyWakeUp (void)
  {
    m_txop->NotifyWakeUp ();
 	...
 	// NETSYS --------------------------------------------------------------------
	virtual bool DoGetToken(void) {
		return m_txop->GetToken();
	}	
	//----------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
void
EdcaTxopN::SetGotAckCallback(EdcaTxopNGotAck callback) {
	NS_LOG_FUNCTION(this << &callback);
	m_gotAckCallback = callback;
}

void
EdcaTxopN::SetMissedAckCallback(EdcaTxopNMissedAck callback) {
	NS_LOG_FUNCTION(this << &callback);
	m_missedAckCallback = callback;
}

void
EdcaTxopN::SetToken(bool cmd) {
	NS_LOG_FUNCTION(this << cmd);

	bool obtain_token = false;
	if(m_token == false && cmd == true) {
		obtain_token = true;
	}
	...
	if(obtain_token) {
		//NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> RESET BACKOFF");
		m_manager->ResetBackoff();
	}
}
...
{
	//NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> NotifyAccessGranted");
  ...
  if (m_currentPacket == 0)
    {
      if (m_queue->IsEmpty () && !m_baManager->HasPackets ())
        {
			//NS_LOG_UNCOND("NotifyAccessGranted: queue is empty");
          NS_LOG_DEBUG ("queue is empty");
          return;
        }
      if (m_baManager->HasBar (m_currentBar))
        {
			//NS_LOG_UNCOND("NotifyAccessGranted: send block ack request");
      ...
      if (m_currentPacket == 0)
        {
          if (m_queue->PeekFirstAvailable (&m_currentHdr, m_currentPacketTimestamp, m_qosBlockedDestinations) == 0)
            {
              //NS_LOG_UNCOND ("no available packets in the queue size: " << m_queue->GetSize());
...
EdcaTxopN::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  m_queue->Enqueue (packet, hdr);
	...
	if(now > Seconds(290.004689850) && now < Seconds(290.004689860)) {

	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> Queue Queued Packets: " << m_queue->GetSize());
...
{
	// NETSYS:DEBUG
	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> GotAck Queued Packets: " << m_queue->GetSize());
...
EdcaTxopN::MissedAck (void)
{
	// NETSYS:DEBUG
	if(m_low->GetDebug()) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> MissedAck");
	}
...
EdcaTxopN::MissedBlockAck (uint32_t nMpdus)
{
	// NETSYS:DEBUG
	if(m_low->GetDebug()) {
		NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> MissedBlockAck");
	}
  ...
  if (NeedBarRetransmission ())
	...
	// NETSYS ------------------------------------------------------------------
	// inform MrWifiNetDevice of this event
...
{
  	NS_LOG_FUNCTION (this);
  	if ((m_currentPacket != 0 || !m_queue->IsEmpty () || m_baManager->HasPackets ()) && !m_dcf->IsAccessRequested ())
    {
      	Ptr<const Packet> packet;
      	WifiMacHeader hdr;
      	if (m_currentPacket != 0) {
		...
		/*
		if(m_low->GetIfIndex() == 1) 
			NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> RestartAccessIfNeeded");
		*/
      	m_manager->RequestReaccess(m_dcf);
    }
}
...
EdcaTxopN::StartAccessIfNeeded (void)
{
  NS_LOG_FUNCTION (this);
		...
		if(m_low->GetDebug()) {
			NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> StartAccessIfNeeded " << m_dcf->IsAccessRequested());
			//NS_LOG_UNCOND("m_currentPacket: " << m_currentPacket << " m_queue->IsEmpty(): " << m_queue->IsEmpty() << " m_baManager->HasPackets(): " << m_baManager->HasPackets() << " m_dcf->IsAccessRequested(): " << m_dcf->IsAccessRequested());
		}
...
EdcaTxopN::PushFront (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  m_queue->PushFront (packet, hdr);
	...
	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> PushFront Size: " << m_queue->GetSize());
...
EdcaTxopN::GotAddBaResponse (const MgtAddBaResponseHeader *respHdr, Mac48Address recipient)
{
  ...
  NS_LOG_DEBUG ("received ADDBA response from " << recipient);
	...
	// NETSYS ------------------------------------------------------------------
	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> received ADDBA response from " << recipient);
	//--------------------------------------------------------------------------
...
EdcaTxopN::GotDelBaFrame (const MgtDelBaHeader *delBaHdr, Mac48Address recipient)
{
	...
	// NETSYS -----------------------------------------------------------------
	NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> received DELBA from " << recipient);
	//--------------------------------------------------------------------------
...
EdcaTxopN::GotBlockAck (const CtrlBAckResponseHeader *blockAck, Mac48Address recipient, double rxSnr, WifiMode txMode, double dataSnr)
{
	...
	if(m_low->GetDebug()) {
		if(!GetToken()) {
			NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> GotBlockAck New Backoff: " << m_backoffTrace << " Queued Packets: " << m_queue->GetSize());
		} else {
			NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> GotBlockAck Token Access: " << m_backoffTrace << " Queued Packets: " << m_queue->GetSize());
		}
	}
...
{
	// NETSYS ------------------------------------------------------------------
	if(m_low->GetDebug()) NS_LOG_UNCOND(Simulator::Now() << " " << m_low->GetAddress() << " <" << m_low->GetIfIndex() << "> sent ADDBA request to " << dest);
	//--------------------------------------------------------------------------
