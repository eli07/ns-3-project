                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  //NS_LOG_UNCOND (Simulator::Now() << " Packet from " << from << " received on node " << m_node->GetId ());
...
Ipv4L3Protocol::LocalDeliver (Ptr<const Packet> packet, Ipv4Header const&ip, uint32_t iif)
{
  ...
  m_localDeliverTrace (ipHeader, p, iif);
	...
	//NS_LOG_UNCOND(Simulator::Now() << " Ipv4L3Protocol::LocalDeliver");
      ...
      switch (status) {
        ...
        case IpL4Protocol::RX_ENDPOINT_UNREACH:
          if (ipHeader.GetDestination ().IsBroadcast () == true ||
			...
			//NS_LOG_UNCOND(Simulator::Now() << " Ipv4L3Protocol::RX_ENDPOINT_UNREACH");
